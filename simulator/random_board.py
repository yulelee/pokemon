...
class Random_board(object):
    ...
    def _random_nearby_positions(self, current_position, num_nearby):
        ...
        def _axis_within_board(current):
            propose = random.gauss(current, self.nearby_variance)
            if propose < 0: 
                return _axis_within_board(current - propose)
            if propose >= self.board_size:
                return _axis_within_board(current - (propose - self.board_size))
            return int(propose)
